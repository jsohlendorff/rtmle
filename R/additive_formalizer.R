...
additive_formalizer <- function(x,
    ...
    K = length(x$times)
    # baseline propensity 
    propensity_formulas <- c(unlist(lapply(x$name_regimen,function(reg){
        paste0(reg,"_0"," ~ ",get_rhs(timepoint = 0, work_data = x$data,name_baseline_covariates = x$name_baseline_covariates,name_time_covariates = x$name_time_covariates,name_regimen = reg, regimen = FALSE,Markov = Markov, constant_variables = x$constant_variables))
    })))
    # censoring in first interval 
    if(length(x$name_censoring)>0){
        censoring_formulas <- paste0(x$name_censoring,"_1"," ~ ", get_rhs(timepoint = 0,work_data = x$data,name_baseline_covariates = x$name_baseline_covariates,name_time_covariates = x$name_time_covariates,name_regimen = x$name_regimen,regimen = TRUE,Markov = Markov,constant_variables = x$constant_variables))
    } else {
    ...
    if(K>1){
        propensity_formulas <- c(propensity_formulas,unlist(lapply(x$times[-1],function(tk){
            c(unlist(lapply(x$name_regimen, function(reg){
                paste0(reg,"_",tk," ~ ",get_rhs(timepoint = tk, work_data = x$data,name_baseline_covariates = x$name_baseline_covariates,name_time_covariates  = x$name_time_covariates,name_regimen = reg, regimen = TRUE,Markov = Markov, constant_variables = x$constant_variables))
            })))
        })))
        if(length(x$name_censoring)>0){
            censoring_formulas <- c(censoring_formulas,unlist(lapply(x$times[-c(1,2)],function(tk){
                paste0(x$name_censoring,"_",tk," ~ ",get_rhs(timepoint = tk, work_data = x$data,name_baseline_covariates = x$name_baseline_covariates,name_time_covariates = x$name_time_covariates,name_regimen = x$name_regimen, regimen = TRUE,Markov = Markov, constant_variables = x$constant_variables))
            })))
        }
    }
    ...
    outcome_formulas <- unlist(lapply(x$times[-1],function(tk){
        paste0(x$name_outcome,"_",tk," ~ ", get_rhs(timepoint = tk, work_data = x$data,
                                                    name_baseline_covariates = x$name_baseline_covariates,
                                                    name_time_covariates  = x$name_time_covariates, 
                                                    name_regimen = x$name_regimen, regimen = TRUE,
    ...
    propensity_formulas <- propensity_formulas[names(propensity_formulas) %in% names(x$data)]
    censoring_formulas <- censoring_formulas[names(censoring_formulas) %in% names(x$data)]
